...
{
    public class ConfectionWorldCreationUI
    {
        ...
        private static void ConfectionMenu()
        {
            UIList optionsList = new UIList();
            optionsList.Add(new UI.ListItem("Confection", TheConfectionRebirth.mod.GetTexture("Sprites/ConfectionTreeIcon"), delegate {
                ConfectionWorldGeneration.hallowMenuSelection = ConfectionWorldGeneration.HallowVariant.confection;
                FinishedMenu();
            }));
            optionsList.Add(new UI.ListItem("Hallow", TheConfectionRebirth.mod.GetTexture("Sprites/HallowTreeIcon"), delegate {
                ConfectionWorldGeneration.hallowMenuSelection = ConfectionWorldGeneration.HallowVariant.hallow;
                FinishedMenu();
            }));
            optionsList.Add(new UI.ListItem("Random", TheConfectionRebirth.mod.GetTexture("Sprites/BothTrees"), delegate {
                ConfectionWorldGeneration.hallowMenuSelection = ConfectionWorldGeneration.HallowVariant.random;
                FinishedMenu();
            }));;
            Main.MenuUI.SetState(new UI.GenericSelectionMenu("Pick Hardmode good", optionsList, delegate {
                if (isFirst)
                {
                    Main.menuMode = -7;
                }

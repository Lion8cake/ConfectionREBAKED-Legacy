...
namespace TheConfectionRebirth.NPCs
{
	internal class GummyWyrmHead : ConfectionWorm
	{
        ...
        public override float SpawnChance(NPCSpawnInfo spawnInfo)
        {
		 if(spawnInfo.player.ZoneRockLayerHeight && spawnInfo.player.GetModPlayer<ConfectionPlayer>().ZoneConfection)
		   return spawnInfo.player.ZoneRockLayerHeight && spawnInfo.player.GetModPlayer<ConfectionPlayer>().ZoneConfection ? .05f : 0f;
		 return 0;
        }
		...
		public override void HitEffect(int hitDirection, double damage) {
			if (npc.life <= 0) {
				...
				NPC.NewNPC((int)spawnAt.X, (int)spawnAt.Y, ModContent.NPCType<GummyWyrmMini1Head>());
				NPC.NewNPC((int)spawnAt.X, (int)spawnAt.Y, ModContent.NPCType<GummyWyrmMini1Head>());
			}
		}
    }

    internal class GummyWyrmBody : ConfectionWorm
    {
    ...
    public abstract class ConfectionWorm : Worm
    {
        ...
        public override void Init()
        {
            minLength = 12;
            maxLength = 18;
            tailType = ModContent.NPCType<GummyWyrmTail>();
            bodyType = ModContent.NPCType<GummyWyrmBody>();
            headType = ModContent.NPCType<GummyWyrmHead>();
            speed = 8.5f;
            turnSpeed = 0.07f;
        }
    }

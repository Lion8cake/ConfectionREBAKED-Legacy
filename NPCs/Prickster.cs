...
namespace TheConfectionRebirth.NPCs 
{
	public class Prickster : ModNPC
	{
		...
		{
			if (spawnInfo.player.GetModPlayer<ConfectionPlayer>().ZoneConfection && spawnInfo.player.ZoneRockLayerHeight)
				return spawnInfo.player.GetModPlayer<ConfectionPlayer>().ZoneConfection && spawnInfo.player.ZoneRockLayerHeight ? .05f : 0f;
			return 0;
		}
        ...
        {
			if (Main.netMode != NetmodeID.MultiplayerClient) // should do this before using random values in multiplayer!!!
			{
				Item.NewItem(npc.position, mod.ItemType("Saccharite") /*saccharite crystal*/, Main.rand.Next(5, 13));
				if (Main.rand.Next(0, 100) == 0) // 1% chance of dropping
					Item.NewItem(npc.position, 885 /*adhesive bandage*/, 1);
			}
		}

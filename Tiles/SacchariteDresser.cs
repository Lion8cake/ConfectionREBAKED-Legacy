...
namespace TheConfectionRebirth.Tiles
{
	public class SacchariteDresser : ModTile
	{
		public override void SetDefaults() {
			...
			name.SetDefault("Saccharite Dresser");
			AddMapEntry(new Color(32, 174, 221), name);
			disableSmartCursor = true;
			adjTiles = new int[] { TileID.Dressers };
			dresser = "Saccharite Dresser";
			dresserDrop = ModContent.ItemType<Items.Placeable.SacchariteDresser>();
		...
		public override void MouseOverFar(int i, int j) {
			...
			if (chestIndex < 0) {
				...
				if (player.showItemIconText == chest) {
					player.showItemIcon2 = ModContent.ItemType<Items.Placeable.SacchariteDresser>();
